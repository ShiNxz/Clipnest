...
type Props = {
	...
	onToggle: () => void
	size?: number
	/** Bare number instead of "3 likes" — for the tight rows under a comment. */
	compact?: boolean
}
			...
			{likeCount > 0 && (
				<HoverCard width={240} shadow="md" openDelay={120} closeDelay={80} position="bottom-start" withArrow>
					<HoverCard.Target>
						...
						<span
							className={`cursor-default select-none font-medium text-slate-300 ${compact ? 'text-xs' : 'text-sm'}`}
						>
